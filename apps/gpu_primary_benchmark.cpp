...
[[nodiscard]] std::string makeGpuOnlySummary(
    ...
    const std::size_t forcedOpaqueMaterialCount,
    const GpuRun& gpu,
    ...
    output << std::fixed << std::setprecision(6)
           ...
           << "GPU cutout fallback: " << forcedOpaqueMaterialCount
           << " material slots treated as opaque for this geometry-only run\n"
    ...
    output << "\nGPU linear-image validation\n"
           ...
           << "beauty render. The benchmark-local packed scene treats alpha cutouts as opaque.\n";
    return output.str();
}
...
[[nodiscard]] std::string makeSummary(
    ...
    output << "\nObserved median render-call wall-clock ratios\n"
           ...
           << "  Informal only: CPU and GPU cutout semantics differ in this run.\n\n"
           ...
           << "Caveat: the CPU importer honors alpha cutouts while the current GPU geometry AOV\n"
           << "treats those triangles as opaque. This is a Bistro traversal wall-clock check,\n"
           << "not yet a textured beauty-render comparison.\n";
    return output.str();
}
...
int main(int argc, char** argv) {
    try {
        ...
        std::size_t forcedOpaqueMaterialCount = 0U;
        for (PackedMaterial& material : scene.materials) {
            if ((material.flagsAndReserved[0] & kPackedMaterialCutout) != 0U) {
                material.flagsAndReserved[0] &= ~kPackedMaterialCutout;
                ++forcedOpaqueMaterialCount;
            }
        }
